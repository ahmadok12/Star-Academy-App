...
export default function ExpenseBreakdownReport({
  ...
  return (
    <div className="space-y-4 pb-24 animate-in fade-in duration-200 w-full min-w-0 overflow-x-hidden p-3.5">
      ...
      <div className="bg-white rounded-3xl border border-[#E5E7EB] shadow-[0_4px_24px_-2px_rgba(17,24,39,0.04)] p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {categoryBreakdown.map((cat) => (
            <div key={cat.category} className="p-3.5 bg-[#F8F9FB] rounded-2xl border border-[#E5E7EB] space-y-2">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-xs text-slate-900">{cat.category}</h4>
                  <span className="text-[10px] text-[#575E70] font-medium">{cat.count} Transactions</span>
                </div>
                <div className="text-right">
                  <span className="text-xs font-bold text-slate-900 font-display">Rs. {cat.total.toLocaleString()}</span>
            ...
            <tbody className="divide-y divide-[#F3F4F6] font-medium">
              {filteredExpenses.length === 0 ? (
                ...
                filteredExpenses.map(e => (
                  <tr key={e.id} className="hover:bg-[#F8F9FB]/80 transition-colors">
                    <td className="py-2.5 px-3 text-[#575E70]">{e.date}</td>
                    ...
                    <td className="py-2.5 px-3 text-[#575E70]">{bankMap[e.bankId] || e.bankName || 'Cash'}</td>
                    <td className="py-2.5 px-3 text-right font-black text-rose-700 font-display">

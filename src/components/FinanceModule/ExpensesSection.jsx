      ...
      <div className="flex items-center justify-between gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3.5 py-2 bg-slate-100 hover:bg-slate-100/80 focus:bg-white text-xs rounded-xl border border-transparent focus:border-rose-500 focus:ring-2 focus:ring-rose-100 transition-all outline-none"
          />
        </div>
        ...
        {activeTab === 'charged' ? (
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 active:scale-98 text-white font-bold text-xs shadow-xs transition-all shrink-0"
          >
        ...
        <div className="space-y-2.5">
          {filteredCharged.length === 0 ? (
            ...
            filteredCharged.map((chg) => (
              <div
                key={chg.id}
                className="p-3.5 bg-white rounded-2xl border border-slate-200/80 shadow-xs hover:border-rose-200 transition-all flex items-center justify-between gap-3"
              >
                <div className="w-10 h-10 rounded-xl bg-rose-50 border border-rose-100 flex items-center justify-center text-rose-600 shrink-0 font-bold">
                ...
                <div className="text-right shrink-0 flex flex-col items-end gap-1.5">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-800 text-[11px] font-bold transition-colors"
                  >
          ...
          {filteredCategories.length === 0 ? (
            ...
            filteredCategories.map((cat) => {
              ...
              return (
                <div
                  key={cat.id}
                  className="p-3.5 bg-white rounded-2xl border border-slate-200/80 shadow-xs hover:border-amber-200 transition-all flex items-center justify-between gap-3"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[10px] text-amber-700 font-bold">{count} vouchers charged</span>
                  </div>

                  {/* VIEW BUTTON */}
                  <button
                    onClick={() => setViewingCategory(cat)}
                    className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-900 text-[11px] font-bold transition-colors shrink-0"
                  >

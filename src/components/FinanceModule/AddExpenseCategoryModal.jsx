...
import { generateNextExpenseCategoryId } from '../../utils/storage';

export default function AddExpenseCategoryModal({ isOpen, onClose, onAddCategory, categories }) {
  const nextId = generateNextExpenseCategoryId(categories);
      ...
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh]">
        ...
        <div className="bg-gradient-to-r from-amber-600 to-orange-600 p-5 text-white flex items-center justify-between shadow-md">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-black tracking-tight leading-tight">Add Expense Head</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 text-xs">
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl bg-amber-50/70 border border-amber-200/60">
            ...
            <span className="px-2.5 py-1 rounded-lg bg-amber-700 text-white font-mono font-bold text-xs tracking-wider">
              {nextId}
            </span>
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="e.g. Electricity & Generator, Printing & Stationary..."
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              className={`w-full px-3.5 py-2.5 rounded-xl border ${
                errors.name ? 'border-rose-400 bg-rose-50/50' : 'border-slate-200 focus:border-amber-500'
              } text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-amber-100 outline-none`}
            />
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea
                rows={3}
                placeholder="Describe what items fall under this expense head..."
                value={formData.details}
                onChange={(e) => setFormData({ ...formData, details: e.target.value })}
                className={`w-full pl-9 pr-3 py-2 rounded-xl border ${
                  errors.details ? 'border-rose-400 bg-rose-50/50' : 'border-slate-200 focus:border-amber-500'
                } text-xs font-medium focus:ring-2 focus:ring-amber-100 outline-none`}
              />
            </div>

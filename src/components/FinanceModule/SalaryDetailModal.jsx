...
import { X, Banknote, Edit3, Trash2, Calendar, Landmark, User, FileText, AlertTriangle } from 'lucide-react';
...
export default function SalaryDetailModal({
  ...
  const paidBank = banks.find(b => b.id === salary.bankId);
  const teacherObj = teachers.find(t => t.id === salary.teacherId);
        ...
        <div className="p-5 overflow-y-auto space-y-4 text-xs">
          ...
          <div className="space-y-2.5 bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <div className="flex items-center justify-between py-1 border-b border-slate-200/60">
              ...
              <span className="font-bold text-slate-800 text-right">{salary.teacherName}</span>
            </div>
            ...
            {teacherObj?.department && (
              <div className="flex items-center justify-between py-1 border-b border-slate-200/60">
                ...
                <span className="font-medium text-slate-700 text-right">{teacherObj.department}</span>
              </div>
            )}
            ...
            <div className="flex items-center justify-between py-1 border-b border-slate-200/60">
              ...
              <span className="font-bold text-slate-800 text-right">
                {paidBank ? paidBank.bankName : salary.bankId}
              </span>
            </div>
          ...
          {confirmDelete ? (
            <div className="p-3 bg-rose-50 border border-rose-200 rounded-2xl space-y-2">
              <div className="flex items-center gap-2 text-rose-700 font-bold text-xs">
                ...
                <span>Delete this payroll voucher?</span>
              </div>
              <p className="text-[11px] text-rose-600">
                Are you sure? This will delete salary payment record {salary.id}.
              </p>

import React, { useState } from 'react';
import { X, Receipt, Edit3, Trash2, Calendar, Landmark, Tag, FileText, AlertTriangle } from 'lucide-react';

export default function ChargedExpenseDetailModal({
  isOpen,
  onClose,
  chargedExpense,
  banks,
  onEdit,
  onDelete
}) {
  const [confirmDelete, setConfirmDelete] = useState(false);

  if (!isOpen || !chargedExpense) return null;

  const paidBank = banks.find(b => b.id === chargedExpense.bankId);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh]">
        {/* Header with EDIT AT TOP */}
        <div className="bg-gradient-to-r from-rose-600 to-red-700 p-5 text-white flex items-center justify-between shadow-md">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center">
              <Receipt className="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 className="text-base font-black tracking-tight leading-tight">Expense Voucher</h2>
              <p className="text-[11px] text-rose-100 font-mono font-medium">{chargedExpense.id}</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            {/* EDIT BUTTON AT TOP */}
            <button
              onClick={() => {
                onClose();
                onEdit(chargedExpense);
              }}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white text-xs font-bold transition-colors"
            >
              <Edit3 className="w-3.5 h-3.5" />
              <span>Edit</span>
            </button>

            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 backdrop-blur-sm flex items-center justify-center text-white transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>

        {/* Content */}
        <div className="p-5 overflow-y-auto space-y-4 text-xs">
          {/* Amount Paid Card */}
          <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200/80 text-center space-y-1">
            <span className="text-[10px] font-bold text-rose-600 uppercase tracking-wider">
              Charged Expense Amount
            </span>
            <div className="text-2xl font-black text-rose-900 tracking-tight">
              Rs. {Number(chargedExpense.amount).toLocaleString()}
            </div>
            <span className="text-[10px] text-rose-500 font-medium block">
              Charged on {chargedExpense.date}
            </span>
          </div>

          {/* Details Card */}
          <div className="space-y-2.5 bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <div className="flex items-start justify-between py-1 border-b border-slate-200/60">
              <span className="text-slate-500 font-medium">Expense Category</span>
              <span className="font-bold text-slate-800 text-right max-w-[60%]">
                {chargedExpense.expenseCategoryName}
              </span>
            </div>

            <div className="flex items-start justify-between py-1 border-b border-slate-200/60">
              <span className="text-slate-500 font-medium">Payment Account</span>
              <span className="font-bold text-slate-800 text-right">
                {paidBank ? paidBank.bankName : chargedExpense.bankId}
              </span>
            </div>

            <div>
              <span className="text-slate-400 font-medium block text-[10px] uppercase mb-1">
                Voucher Purpose / Remarks
              </span>
              <p className="font-medium text-slate-800 text-xs bg-white p-2.5 rounded-xl border border-slate-200 whitespace-pre-wrap">
                {chargedExpense.details || 'No remarks added'}
              </p>
            </div>
          </div>
        </div>

        {/* Footer with DELETE BUTTON AT BOTTOM */}
        <div className="p-4 bg-slate-50 border-t border-slate-100">
          {confirmDelete ? (
            <div className="p-3 bg-rose-50 border border-rose-200 rounded-2xl space-y-2">
              <div className="flex items-center gap-2 text-rose-700 font-bold text-xs">
                <AlertTriangle className="w-4 h-4 text-rose-600" />
                <span>Delete this expense voucher?</span>
              </div>
              <p className="text-[11px] text-rose-600">
                Are you sure? This will delete expense record {chargedExpense.id}.
              </p>
              <div className="flex gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => setConfirmDelete(false)}
                  className="flex-1 py-1.5 px-3 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={() => {
                    onDelete(chargedExpense.id);
                    onClose();
                  }}
                  className="flex-1 py-1.5 px-3 rounded-xl bg-rose-600 text-white font-bold text-xs shadow-xs hover:bg-rose-700"
                >
                  Confirm Delete
                </button>
              </div>
            </div>
          ) : (
            <button
              type="button"
              onClick={() => setConfirmDelete(true)}
              className="w-full py-2.5 px-4 rounded-xl border border-rose-200 bg-rose-50/50 hover:bg-rose-100/60 text-rose-700 font-bold text-xs flex items-center justify-center gap-2 transition-colors"
            >
              <Trash2 className="w-4 h-4 text-rose-600" />
              <span>Delete Expense Voucher</span>
            </button>
          )}
        </div>
      </div>
    </div>
  );
}

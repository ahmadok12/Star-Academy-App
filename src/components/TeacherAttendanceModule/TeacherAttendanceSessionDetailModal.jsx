...
import { X, Calendar, CheckCircle2, XCircle, Clock, Trash2, Edit3, Save, RotateCcw, GraduationCap } from 'lucide-react';
...
export default function TeacherAttendanceSessionDetailModal({
  ...
  const handleSaveEdit = () => {
    const updatedRecords = (session.records || []).map((r) => ({
      ...r,
      status: recordsMap[r.teacherId] || r.status
    }));
            ...
            session.records.map((rec, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Editing Controls (Only current selection is colored) */}
                  {isEditing && (
